...
class WineInfoActivity : AppCompatActivity() {
    ...
    private fun populate() {
            ...
            val volumeString =
                getString(R.string.wine_item_volume, wineModel.volume).replace(',', '.')
            productInfo.text = getString(
                R.string.wine_item_description,
                wineModel.country,
                wineModel.amountOfSugar,
                wineModel.color,
                volumeString
            )
            ...
            if (wineModel.oldPrice == 0f || wineModel.oldPrice == wineModel.price) {
                ...
                oldPrice.text = getString(R.string.wine_item_price, wineModel.oldPrice)
            ...
            if (wineModel.isFavorite) toFavorites.setImageResource(R.drawable.ic_like_red) else toFavorites.setImageResource(R.drawable.ic_like)
    ...
    private fun setSimilarListeners() {
        button_back.setOnClickListener() {
            ...
            if (current == similarRecyclerView.adapter?.itemCount ?: Int) button_forward.alpha =
                0.25F else button_forward.alpha = 1F
